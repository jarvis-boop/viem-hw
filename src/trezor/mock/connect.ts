...
export class MockTrezorConnect implements TrezorConnect {
  ...
  async ethereumSignMessage(
    ...
    // Sign using viem
    const message = params.hex ? { raw: `0x${params.message}` as HexType } : params.message;
   ...
   */
  private getScenario(
    operation: keyof NonNullable<MockTrezorOptions["scenarioOverrides"]>,
  ): MockTrezorScenario {
   ...
   */
  private getAccountForPath(path: string): {
    address: string;
    publicKey: string;
    privateKey: string;
  } {
