...
export class MockLedgerEthApp {
  ...
  async signTransaction(
    ...
    const message = { raw: `0x${rawTxHex.replace(/^0x/, '').slice(0, 64).padEnd(64, '0')}` as HexType }
    const signature = await viemAccount.signMessage({ message })
   ...
   */
  private checkScenario(operation: 'getAddress' | 'signMessage' | 'signTransaction' | 'signTypedData'): void {
    if (!this.transport.isConnected()) {
      throw { name: 'TransportOpenUserCancelled', message: 'Device disconnected' }
    }
