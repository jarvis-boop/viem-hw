...
function App() {
  ...
  const [appConfig, setAppConfig] = useState<LedgerAppConfig | null>(null)
  ...
  const connectLedger = async () => {
    ...
    try {
      ...
      setDeviceInfo(info)
      log('success', `✅ Connected: ${info.model || 'Ledger'} (FW: ${info.firmwareVersion || 'unknown'})`)

      try {
        const config = await manager.getAppConfig()
        setAppConfig(config)
        log('info', `📱 Ethereum app v${config.version} (EIP-712: ${config.supportsEIP712 ? 'yes' : 'no'})`)
      } catch {
  ...
  const verifyAddress = async () => {
    ...
    try {
      const manager = createLedgerDeviceManager()
      await manager.connect()
      const { address, verified } = await manager.verifyAddress(accounts.find(a => a.address === selectedAccount.address)?.path)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="card">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setLogs([])} className="text-sm text-[var(--muted)] hover:text-[var(--fg)]">
              Clear
            </button>
          </div>
              ...
              logs.map((l) => (
                <div
                  ...
                  className={`${
                    ...
                    l.type === 'success' ? 'text-[var(--success)]' :
                    'text-[var(--muted)]'
                  }`}
                >
                  <span className="opacity-50">{l.time.toLocaleTimeString()}</span>{' '}
                  {l.message}
                </div>
              ))
            )}
